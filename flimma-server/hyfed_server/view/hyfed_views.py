...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True)
    def volcano_plot(self, request, *args, **kwargs):
        try:
            project_instance = self.get_object()
            project_id = project_instance.id
            plot_file_path = f'{project_instance.result_dir}/{project_id}/p{project_id}_volcano.png'

            http_response = HttpResponse(FileWrapper(open(plot_file_path, 'rb')), content_type='image/png')
